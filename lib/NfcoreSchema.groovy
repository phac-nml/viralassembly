...
class NfcoreSchema {
    ...
    public static String paramsHelp(workflow, params, command, schema_filename='nextflow_schema.json') {
        ...
        String output  = ''
        output        += 'Example pipeline command:\n\n'
        ...
        // Adding in type colours for fun
        Map type_colors = [:]
        type_colors['[boolean]'] = colors.blue
        type_colors['[string]'] = colors.purple
        type_colors['[integer]'] = colors.cyan
        type_colors['[number]'] = colors.green
        for (group in params_map.keySet()) {
            ...
            for (param in group_params.keySet()) {
                ...
                group_output += "  --" +  param.padRight(max_chars) + type_colors[type] + type.padRight(10) + colors.reset + description_default + '\n'
                num_params += 1
            }
            group_output += '\n'
            if (num_params > 0){
