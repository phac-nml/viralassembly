...
df <- summary_df %>% 
  select(any_of(c('sample', 'qc_pass', 'mean_sequencing_depth', 'median_sequencing_depth', 
                  ...
                  'genome_completeness', 'minor_snps', 'minor_indels')))
...
if ("minor_snps" %in% colnames(df)) {
    ...
    rename(
      ...
      "# Subconsensus SNPs" = minor_snps,
      "# Subconsensus Indels" = minor_indels
    )
} else {
  df <- df %>%
    rename(
...
for ( samplev in summary_df$sample ) {
  ...
  assign("min_allele_freq", params$min_allele_freq, subpage_env)
